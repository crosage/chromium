...
class PowerShellService {
  static Future<CommandLog> execute(
    ...
    try {
      ...
      logData['exitCode'] = processResult.exitCode;

      if (kDebugMode) {
        print("PowerShell 退出码: ${logData['exitCode']}");
        if (logData['stdout']!.isNotEmpty) print("PowerShell Stdout:\n${logData['stdout']}");
        if (logData['stderr']!.isNotEmpty) print("PowerShell Stderr:\n${logData['stderr']}");
      }

      if (logData['exitCode'] != 0 && logData['stderr']!.isEmpty) {
        logData['stderr'] = '(命令以非零状态 ${logData['exitCode']} 退出，但标准错误无输出。)';
      } else if (logData['exitCode'] != 0 && logData['stderr']!.isNotEmpty) {
        logData['stderr'] = (logData['stderr'] ?? '') + '\n(命令以非零状态 ${logData['exitCode']} 退出。)';
      }
